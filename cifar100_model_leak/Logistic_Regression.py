...
INPUT_PATH = '../data/cifar100/leak/all.csv'
LABEL_PATH = '../data/cifar100/leak/all_label.csv'
...
# 構建 Logistic Regression 模型來預測年齡
...
end_time = time.time()
count_time('train acc model',start_time,end_time)

# 構建 Logistic Regression 模型來預測性別
start_time = time.time()
budget_model = LogisticRegression()
budget_model.fit(x_train, train_Y_budget)
end_time = time.time()
count_time('train budget model',start_time,end_time)

# 進行預測
predicted_budget = budget_model.predict(x_test)  # 預測年齡
predicted_acc = acc_model.predict(x_test)  # 預測性別
...
for n in range(len(x_test)):
    pre_budget = predicted_budget[n]  # 預測的年齡
    pre_acc = predicted_acc[n]  # 預測的性別
